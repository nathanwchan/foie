...
7. Add source-native media. Use an inspectable YouTube or first-party video when the chosen resource is a video or a video-recorded podcast. For an X post containing video, use the official X embed media type with the canonical post ID and its poster image; never extract or store a raw `video.twimg.com` delivery URL. Otherwise use the chosen source's relevant Open Graph image or a directly relevant figure. Never generate a generic placeholder or use a companion URL merely to make the card look like a different format.
