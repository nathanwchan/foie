...
3. Before accepting a resource in a scheduled run, reject it if its canonical destination is a repository, tool or product page, package listing, release, app listing, or direct tool documentation. Only an explicit curator request naming that exact artifact may override this scheduled-automation rule.
4. Publish exactly one `canonicalUrl` per resource. When several versions exist, choose the most complete, original, stable, and directly inspectable editorial destination. Prefer the author or publisher's first-party article or recording when it contains the full material or transcript; choose the direct YouTube, podcast, paper, or social URL when that is the primary editorial artifact. The resource `format` must describe this chosen URL. Keep equivalent mirrors and companion repositories only as private ledger aliases for deduplication; never add them to a resource or render them publicly. Keep materially distinct editorial companions as separate related resources when each independently meets the publication threshold.
5. Verify title, publisher, author, original publication date, and availability from the source. Use a null publication date only when the source does not expose one.
6. Write a source-grounded summary of two or three sentences that begins with the exact name in `authors[0].name`, followed by one specific action an experienced iOS team can take. Preserve source uncertainty and avoid unsupported claims.
